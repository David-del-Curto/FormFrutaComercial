/*
    Form Fruta Comercial - Azure SQL DW bootstrap
    ------------------------------------------------
    Este script crea un esquema sencillo de staging + DW para poblar
    formularios y defectos desde la app o desde un ETL intermedio.

    Recomendacion clave:
    - source_business_key debe ser una clave global, estable e inmutable
      por formulario (idealmente un GUID generado por la app o el ETL).
    - No uses solo id_registro local como llave global si hay mas de un
      equipo o dispositivo capturando datos.
*/

IF NOT EXISTS (SELECT 1 FROM sys.schemas WHERE name = 'stg')
BEGIN
    EXEC('CREATE SCHEMA stg');
END
GO

IF NOT EXISTS (SELECT 1 FROM sys.schemas WHERE name = 'dw')
BEGIN
    EXEC('CREATE SCHEMA dw');
END
GO

IF OBJECT_ID('stg.FormularioHeader', 'U') IS NULL
BEGIN
    CREATE TABLE stg.FormularioHeader (
        source_system           NVARCHAR(50)    NOT NULL,
        source_business_key     NVARCHAR(100)   NOT NULL,
        source_record_id        BIGINT          NULL,
        fecha                   DATE            NOT NULL,
        fecha_operacional       DATE            NOT NULL,
        turno_codigo            VARCHAR(10)     NOT NULL,
        turno_nombre            NVARCHAR(50)    NOT NULL,
        rango_turno             NVARCHAR(50)    NOT NULL,
        linea_codigo            NVARCHAR(50)    NOT NULL,
        linea_nombre            NVARCHAR(120)   NULL,
        especie                 NVARCHAR(120)   NOT NULL,
        especie_principal_linea NVARCHAR(120)   NULL,
        variedad                NVARCHAR(120)   NOT NULL,
        lote                    NVARCHAR(50)    NOT NULL,
        centro_codigo           NVARCHAR(20)    NULL,
        centro_nombre           NVARCHAR(200)   NULL,
        centro_display          NVARCHAR(250)   NULL,
        productor_codigo        NVARCHAR(50)    NULL,
        productor_nombre        NVARCHAR(200)   NULL,
        productor_display       NVARCHAR(250)   NULL,
        lugar_codigo            VARCHAR(10)     NULL,
        lugar_nombre            NVARCHAR(100)   NULL,
        verificador             NVARCHAR(100)   NULL,
        observaciones           NVARCHAR(1000)  NULL,
        cant_muestra            INT             NOT NULL,
        suma_defectos           INT             NOT NULL,
        fruta_comercial         INT             NOT NULL,
        fruta_sana              INT             NOT NULL,
        choice                  INT             NOT NULL,
        porc_exportable         DECIMAL(9,2)    NOT NULL,
        porc_embalable          DECIMAL(9,2)    NOT NULL,
        porc_choice             DECIMAL(9,2)    NOT NULL,
        porc_descartable        DECIMAL(9,2)    NOT NULL,
        porc_export_manual      INT             NULL,
        velocidad_kgh           DECIMAL(18,2)   NULL,
        velocidad_manual        DECIMAL(18,2)   NULL,
        centro_sin_definir      BIT             NOT NULL,
        estado_formulario       NVARCHAR(20)    NOT NULL,
        es_completo             BIT             NOT NULL,
        campos_pendientes       NVARCHAR(500)   NULL,
        created_at              DATETIME2(0)    NULL,
        updated_at              DATETIME2(0)    NOT NULL,
        dw_loaded_at            DATETIME2(0)    NULL,
        CONSTRAINT PK_stg_FormularioHeader PRIMARY KEY (source_system, source_business_key)
    );
END
GO

IF OBJECT_ID('stg.FormularioDefecto', 'U') IS NULL
BEGIN
    CREATE TABLE stg.FormularioDefecto (
        source_system           NVARCHAR(50)    NOT NULL,
        source_business_key     NVARCHAR(100)   NOT NULL,
        codigo_defecto          NVARCHAR(20)    NOT NULL,
        nombre_defecto          NVARCHAR(200)   NULL,
        cantidad                INT             NOT NULL,
        updated_at              DATETIME2(0)    NOT NULL,
        dw_loaded_at            DATETIME2(0)    NULL,
        CONSTRAINT PK_stg_FormularioDefecto PRIMARY KEY (
            source_system,
            source_business_key,
            codigo_defecto
        )
    );
END
GO

IF OBJECT_ID('dw.DimCentro', 'U') IS NULL
BEGIN
    CREATE TABLE dw.DimCentro (
        centro_key              INT IDENTITY(1,1) NOT NULL PRIMARY KEY,
        centro_codigo           NVARCHAR(20)      NOT NULL,
        centro_nombre           NVARCHAR(200)     NULL,
        centro_display          NVARCHAR(250)     NULL,
        centro_sin_definir      BIT               NOT NULL DEFAULT 0,
        created_at              DATETIME2(0)      NOT NULL DEFAULT SYSUTCDATETIME(),
        updated_at              DATETIME2(0)      NOT NULL DEFAULT SYSUTCDATETIME()
    );

    CREATE UNIQUE INDEX UX_DimCentro_codigo
        ON dw.DimCentro (centro_codigo);
END
GO

IF OBJECT_ID('dw.DimProductor', 'U') IS NULL
BEGIN
    CREATE TABLE dw.DimProductor (
        productor_key           INT IDENTITY(1,1) NOT NULL PRIMARY KEY,
        productor_codigo        NVARCHAR(50)      NOT NULL,
        productor_nombre        NVARCHAR(200)     NULL,
        productor_display       NVARCHAR(250)     NULL,
        created_at              DATETIME2(0)      NOT NULL DEFAULT SYSUTCDATETIME(),
        updated_at              DATETIME2(0)      NOT NULL DEFAULT SYSUTCDATETIME()
    );

    CREATE UNIQUE INDEX UX_DimProductor_codigo
        ON dw.DimProductor (productor_codigo);
END
GO

IF OBJECT_ID('dw.DimEspecie', 'U') IS NULL
BEGIN
    CREATE TABLE dw.DimEspecie (
        especie_key             INT IDENTITY(1,1) NOT NULL PRIMARY KEY,
        especie_nombre          NVARCHAR(120)     NOT NULL,
        created_at              DATETIME2(0)      NOT NULL DEFAULT SYSUTCDATETIME(),
        updated_at              DATETIME2(0)      NOT NULL DEFAULT SYSUTCDATETIME()
    );

    CREATE UNIQUE INDEX UX_DimEspecie_nombre
        ON dw.DimEspecie (especie_nombre);
END
GO

IF OBJECT_ID('dw.DimVariedad', 'U') IS NULL
BEGIN
    CREATE TABLE dw.DimVariedad (
        variedad_key            INT IDENTITY(1,1) NOT NULL PRIMARY KEY,
        especie_key             INT               NOT NULL,
        variedad_nombre         NVARCHAR(120)     NOT NULL,
        created_at              DATETIME2(0)      NOT NULL DEFAULT SYSUTCDATETIME(),
        updated_at              DATETIME2(0)      NOT NULL DEFAULT SYSUTCDATETIME(),
        CONSTRAINT FK_DimVariedad_DimEspecie
            FOREIGN KEY (especie_key) REFERENCES dw.DimEspecie(especie_key)
    );

    CREATE UNIQUE INDEX UX_DimVariedad_especie_variedad
        ON dw.DimVariedad (especie_key, variedad_nombre);
END
GO

IF OBJECT_ID('dw.DimLinea', 'U') IS NULL
BEGIN
    CREATE TABLE dw.DimLinea (
        linea_key               INT IDENTITY(1,1) NOT NULL PRIMARY KEY,
        centro_codigo           NVARCHAR(20)      NOT NULL,
        linea_codigo            NVARCHAR(50)      NOT NULL,
        linea_nombre            NVARCHAR(120)     NULL,
        especie_principal       NVARCHAR(120)     NULL,
        created_at              DATETIME2(0)      NOT NULL DEFAULT SYSUTCDATETIME(),
        updated_at              DATETIME2(0)      NOT NULL DEFAULT SYSUTCDATETIME()
    );

    CREATE UNIQUE INDEX UX_DimLinea_centro_linea
        ON dw.DimLinea (centro_codigo, linea_codigo);
END
GO

IF OBJECT_ID('dw.DimLugarSeleccion', 'U') IS NULL
BEGIN
    CREATE TABLE dw.DimLugarSeleccion (
        lugar_key               INT IDENTITY(1,1) NOT NULL PRIMARY KEY,
        lugar_codigo            VARCHAR(10)       NOT NULL,
        lugar_nombre            NVARCHAR(100)     NOT NULL,
        created_at              DATETIME2(0)      NOT NULL DEFAULT SYSUTCDATETIME(),
        updated_at              DATETIME2(0)      NOT NULL DEFAULT SYSUTCDATETIME()
    );

    CREATE UNIQUE INDEX UX_DimLugarSeleccion_codigo
        ON dw.DimLugarSeleccion (lugar_codigo);
END
GO

IF OBJECT_ID('dw.DimTurno', 'U') IS NULL
BEGIN
    CREATE TABLE dw.DimTurno (
        turno_key               INT IDENTITY(1,1) NOT NULL PRIMARY KEY,
        turno_codigo            VARCHAR(10)       NOT NULL,
        turno_nombre            NVARCHAR(50)      NOT NULL,
        rango_turno             NVARCHAR(50)      NOT NULL,
        created_at              DATETIME2(0)      NOT NULL DEFAULT SYSUTCDATETIME(),
        updated_at              DATETIME2(0)      NOT NULL DEFAULT SYSUTCDATETIME()
    );

    CREATE UNIQUE INDEX UX_DimTurno_codigo
        ON dw.DimTurno (turno_codigo);
END
GO

IF OBJECT_ID('dw.DimDefecto', 'U') IS NULL
BEGIN
    CREATE TABLE dw.DimDefecto (
        defecto_key             INT IDENTITY(1,1) NOT NULL PRIMARY KEY,
        codigo_defecto          NVARCHAR(20)      NOT NULL,
        nombre_defecto          NVARCHAR(200)     NOT NULL,
        created_at              DATETIME2(0)      NOT NULL DEFAULT SYSUTCDATETIME(),
        updated_at              DATETIME2(0)      NOT NULL DEFAULT SYSUTCDATETIME()
    );

    CREATE UNIQUE INDEX UX_DimDefecto_codigo
        ON dw.DimDefecto (codigo_defecto);
END
GO

IF OBJECT_ID('dw.FactFormulario', 'U') IS NULL
BEGIN
    CREATE TABLE dw.FactFormulario (
        formulario_key          BIGINT IDENTITY(1,1) NOT NULL PRIMARY KEY,
        source_system           NVARCHAR(50)         NOT NULL,
        source_business_key     NVARCHAR(100)        NOT NULL,
        source_record_id        BIGINT               NULL,
        fecha                   DATE                 NOT NULL,
        fecha_operacional       DATE                 NOT NULL,
        turno_key               INT                  NOT NULL,
        centro_key              INT                  NULL,
        productor_key           INT                  NULL,
        especie_key             INT                  NOT NULL,
        variedad_key            INT                  NOT NULL,
        linea_key               INT                  NOT NULL,
        lugar_key               INT                  NULL,
        lote                    NVARCHAR(50)         NOT NULL,
        verificador             NVARCHAR(100)        NULL,
        observaciones           NVARCHAR(1000)       NULL,
        cant_muestra            INT                  NOT NULL,
        suma_defectos           INT                  NOT NULL,
        fruta_comercial         INT                  NOT NULL,
        fruta_sana              INT                  NOT NULL,
        choice                  INT                  NOT NULL,
        porc_exportable         DECIMAL(9,2)         NOT NULL,
        porc_embalable          DECIMAL(9,2)         NOT NULL,
        porc_choice             DECIMAL(9,2)         NOT NULL,
        porc_descartable        DECIMAL(9,2)         NOT NULL,
        porc_export_manual      INT                  NULL,
        velocidad_kgh           DECIMAL(18,2)        NULL,
        velocidad_manual        DECIMAL(18,2)        NULL,
        centro_sin_definir      BIT                  NOT NULL,
        estado_formulario       NVARCHAR(20)         NOT NULL,
        es_completo             BIT                  NOT NULL,
        campos_pendientes       NVARCHAR(500)        NULL,
        created_at              DATETIME2(0)         NULL,
        updated_at              DATETIME2(0)         NOT NULL,
        dw_inserted_at          DATETIME2(0)         NOT NULL DEFAULT SYSUTCDATETIME(),
        dw_updated_at           DATETIME2(0)         NOT NULL DEFAULT SYSUTCDATETIME(),
        CONSTRAINT FK_FactFormulario_DimTurno
            FOREIGN KEY (turno_key) REFERENCES dw.DimTurno(turno_key),
        CONSTRAINT FK_FactFormulario_DimCentro
            FOREIGN KEY (centro_key) REFERENCES dw.DimCentro(centro_key),
        CONSTRAINT FK_FactFormulario_DimProductor
            FOREIGN KEY (productor_key) REFERENCES dw.DimProductor(productor_key),
        CONSTRAINT FK_FactFormulario_DimEspecie
            FOREIGN KEY (especie_key) REFERENCES dw.DimEspecie(especie_key),
        CONSTRAINT FK_FactFormulario_DimVariedad
            FOREIGN KEY (variedad_key) REFERENCES dw.DimVariedad(variedad_key),
        CONSTRAINT FK_FactFormulario_DimLinea
            FOREIGN KEY (linea_key) REFERENCES dw.DimLinea(linea_key),
        CONSTRAINT FK_FactFormulario_DimLugarSeleccion
            FOREIGN KEY (lugar_key) REFERENCES dw.DimLugarSeleccion(lugar_key)
    );

    CREATE UNIQUE INDEX UX_FactFormulario_source_key
        ON dw.FactFormulario (source_system, source_business_key);
END
GO

IF OBJECT_ID('dw.FactFormularioDefecto', 'U') IS NULL
BEGIN
    CREATE TABLE dw.FactFormularioDefecto (
        formulario_key          BIGINT         NOT NULL,
        defecto_key             INT            NOT NULL,
        cantidad                INT            NOT NULL,
        updated_at              DATETIME2(0)   NOT NULL,
        dw_updated_at           DATETIME2(0)   NOT NULL DEFAULT SYSUTCDATETIME(),
        CONSTRAINT PK_FactFormularioDefecto
            PRIMARY KEY (formulario_key, defecto_key),
        CONSTRAINT FK_FactFormularioDefecto_Formulario
            FOREIGN KEY (formulario_key) REFERENCES dw.FactFormulario(formulario_key),
        CONSTRAINT FK_FactFormularioDefecto_Defecto
            FOREIGN KEY (defecto_key) REFERENCES dw.DimDefecto(defecto_key)
    );
END
GO

CREATE OR ALTER PROCEDURE dw.sp_seed_static_catalogs
AS
BEGIN
    SET NOCOUNT ON;

    MERGE dw.DimLugarSeleccion AS tgt
    USING (
        SELECT 'MS' AS lugar_codigo, 'Mesa Seleccion' AS lugar_nombre
        UNION ALL SELECT 'BC', 'Bins Comercial'
        UNION ALL SELECT 'TP', 'Trypack'
    ) AS src
        ON tgt.lugar_codigo = src.lugar_codigo
    WHEN MATCHED THEN
        UPDATE SET
            tgt.lugar_nombre = src.lugar_nombre,
            tgt.updated_at = SYSUTCDATETIME()
    WHEN NOT MATCHED THEN
        INSERT (lugar_codigo, lugar_nombre)
        VALUES (src.lugar_codigo, src.lugar_nombre);

    MERGE dw.DimTurno AS tgt
    USING (
        SELECT 'T1' AS turno_codigo, 'Turno 1' AS turno_nombre, '06:00-14:59' AS rango_turno
        UNION ALL SELECT 'T2', 'Turno 2', '15:00-23:59'
        UNION ALL SELECT 'T3', 'Turno 3', '00:00-05:59'
    ) AS src
        ON tgt.turno_codigo = src.turno_codigo
    WHEN MATCHED THEN
        UPDATE SET
            tgt.turno_nombre = src.turno_nombre,
            tgt.rango_turno = src.rango_turno,
            tgt.updated_at = SYSUTCDATETIME()
    WHEN NOT MATCHED THEN
        INSERT (turno_codigo, turno_nombre, rango_turno)
        VALUES (src.turno_codigo, src.turno_nombre, src.rango_turno);

    MERGE dw.DimDefecto AS tgt
    USING (
        SELECT 'HAO' AS codigo_defecto, 'Herida Abierta (Oxidada)' AS nombre_defecto
        UNION ALL SELECT 'HAF', 'Herida Abierta (Fresca)'
        UNION ALL SELECT 'MAC', 'Machucon'
        UNION ALL SELECT 'PAR', 'Partidura'
        UNION ALL SELECT 'GSS', 'Golpe Sol Severo'
        UNION ALL SELECT 'CRA', 'Cracking'
        UNION ALL SELECT 'DES', 'Deshidratacion'
        UNION ALL SELECT 'DP', 'Desgarro Pedicelar'
        UNION ALL SELECT 'LEN', 'Lenticelosis'
        UNION ALL SELECT 'BP', 'Bitter Pit'
        UNION ALL SELECT 'MR', 'Manchas - Roce'
        UNION ALL SELECT 'RLP', 'Roce (Linea Proceso)'
        UNION ALL SELECT 'FC', 'Falta Color'
        UNION ALL SELECT 'HC', 'Herida Cicatrizada'
        UNION ALL SELECT 'DI', 'Dano Insecto'
        UNION ALL SELECT 'DEF', 'Deforme'
        UNION ALL SELECT 'RAM', 'Ramaleo'
        UNION ALL SELECT 'RG', 'Roce Grave'
        UNION ALL SELECT 'RUS', 'Russet Grave'
    ) AS src
        ON tgt.codigo_defecto = src.codigo_defecto
    WHEN MATCHED THEN
        UPDATE SET
            tgt.nombre_defecto = src.nombre_defecto,
            tgt.updated_at = SYSUTCDATETIME()
    WHEN NOT MATCHED THEN
        INSERT (codigo_defecto, nombre_defecto)
        VALUES (src.codigo_defecto, src.nombre_defecto);

    MERGE dw.DimLinea AS tgt
    USING (
        SELECT 'DC05' AS centro_codigo, 'LIN_01' AS linea_codigo, 'LIN_01' AS linea_nombre, 'PERAS' AS especie_principal
        UNION ALL SELECT 'DC05', 'LIN_02', 'LIN_02', 'MANZANAS'
        UNION ALL SELECT 'DC05', 'LIN_03', 'LIN_03', 'DAGEN'
        UNION ALL SELECT 'DC05', 'LIN_04', 'LIN_04 - MAF', 'CEREZAS'
        UNION ALL SELECT 'DC05', 'LIN_05', 'LIN_05 - UNITEC', 'CEREZAS'
        UNION ALL SELECT 'DC05', 'LIN_06', 'LIN_06', 'DAGEN'
        UNION ALL SELECT 'DC05', 'LIN_07', 'LIN_07', 'PERAS'
        UNION ALL SELECT 'DC10', 'LIN_03', 'LIN_03', 'ARANDANOS'
        UNION ALL SELECT 'DC10', 'LIN_04_G_CAMPO', 'LIN_04_G_CAMPO', 'ARANDANOS GRANEL DE CAMPO'
        UNION ALL SELECT 'DC10', 'REEMBALAJE', 'REEMBALAJE', 'ARANDANOS'
    ) AS src
        ON tgt.centro_codigo = src.centro_codigo
       AND tgt.linea_codigo = src.linea_codigo
    WHEN MATCHED THEN
        UPDATE SET
            tgt.linea_nombre = src.linea_nombre,
            tgt.especie_principal = src.especie_principal,
            tgt.updated_at = SYSUTCDATETIME()
    WHEN NOT MATCHED THEN
        INSERT (centro_codigo, linea_codigo, linea_nombre, especie_principal)
        VALUES (src.centro_codigo, src.linea_codigo, src.linea_nombre, src.especie_principal);
END
GO

CREATE OR ALTER PROCEDURE dw.sp_process_formulario_stage
AS
BEGIN
    SET NOCOUNT ON;

    DECLARE @now DATETIME2(0) = SYSUTCDATETIME();

    ;WITH header_src AS (
        SELECT DISTINCT
            centro_codigo,
            centro_nombre,
            centro_display,
            centro_sin_definir
        FROM stg.FormularioHeader
        WHERE dw_loaded_at IS NULL
          AND NULLIF(LTRIM(RTRIM(centro_codigo)), '') IS NOT NULL
    )
    MERGE dw.DimCentro AS tgt
    USING header_src AS src
        ON tgt.centro_codigo = src.centro_codigo
    WHEN MATCHED THEN
        UPDATE SET
            tgt.centro_nombre = src.centro_nombre,
            tgt.centro_display = src.centro_display,
            tgt.centro_sin_definir = src.centro_sin_definir,
            tgt.updated_at = @now
    WHEN NOT MATCHED THEN
        INSERT (centro_codigo, centro_nombre, centro_display, centro_sin_definir)
        VALUES (src.centro_codigo, src.centro_nombre, src.centro_display, src.centro_sin_definir);

    ;WITH header_src AS (
        SELECT DISTINCT
            productor_codigo,
            productor_nombre,
            productor_display
        FROM stg.FormularioHeader
        WHERE dw_loaded_at IS NULL
          AND NULLIF(LTRIM(RTRIM(productor_codigo)), '') IS NOT NULL
    )
    MERGE dw.DimProductor AS tgt
    USING header_src AS src
        ON tgt.productor_codigo = src.productor_codigo
    WHEN MATCHED THEN
        UPDATE SET
            tgt.productor_nombre = src.productor_nombre,
            tgt.productor_display = src.productor_display,
            tgt.updated_at = @now
    WHEN NOT MATCHED THEN
        INSERT (productor_codigo, productor_nombre, productor_display)
        VALUES (src.productor_codigo, src.productor_nombre, src.productor_display);

    ;WITH header_src AS (
        SELECT DISTINCT especie
        FROM stg.FormularioHeader
        WHERE dw_loaded_at IS NULL
          AND NULLIF(LTRIM(RTRIM(especie)), '') IS NOT NULL
    )
    MERGE dw.DimEspecie AS tgt
    USING header_src AS src
        ON tgt.especie_nombre = src.especie
    WHEN NOT MATCHED THEN
        INSERT (especie_nombre)
        VALUES (src.especie)
    WHEN MATCHED THEN
        UPDATE SET
            tgt.updated_at = @now;

    ;WITH header_src AS (
        SELECT DISTINCT
            h.especie,
            h.variedad
        FROM stg.FormularioHeader h
        WHERE h.dw_loaded_at IS NULL
          AND NULLIF(LTRIM(RTRIM(h.variedad)), '') IS NOT NULL
    )
    MERGE dw.DimVariedad AS tgt
    USING (
        SELECT
            e.especie_key,
            src.variedad
        FROM header_src src
        INNER JOIN dw.DimEspecie e
            ON e.especie_nombre = src.especie
    ) AS src
        ON tgt.especie_key = src.especie_key
       AND tgt.variedad_nombre = src.variedad
    WHEN MATCHED THEN
        UPDATE SET
            tgt.updated_at = @now
    WHEN NOT MATCHED THEN
        INSERT (especie_key, variedad_nombre)
        VALUES (src.especie_key, src.variedad);

    ;WITH header_src AS (
        SELECT DISTINCT
            lugar_codigo,
            lugar_nombre
        FROM stg.FormularioHeader
        WHERE dw_loaded_at IS NULL
          AND NULLIF(LTRIM(RTRIM(lugar_codigo)), '') IS NOT NULL
    )
    MERGE dw.DimLugarSeleccion AS tgt
    USING header_src AS src
        ON tgt.lugar_codigo = src.lugar_codigo
    WHEN MATCHED THEN
        UPDATE SET
            tgt.lugar_nombre = COALESCE(src.lugar_nombre, tgt.lugar_nombre),
            tgt.updated_at = @now
    WHEN NOT MATCHED THEN
        INSERT (lugar_codigo, lugar_nombre)
        VALUES (src.lugar_codigo, COALESCE(src.lugar_nombre, src.lugar_codigo));

    ;WITH header_src AS (
        SELECT DISTINCT
            turno_codigo,
            turno_nombre,
            rango_turno
        FROM stg.FormularioHeader
        WHERE dw_loaded_at IS NULL
          AND NULLIF(LTRIM(RTRIM(turno_codigo)), '') IS NOT NULL
    )
    MERGE dw.DimTurno AS tgt
    USING header_src AS src
        ON tgt.turno_codigo = src.turno_codigo
    WHEN MATCHED THEN
        UPDATE SET
            tgt.turno_nombre = src.turno_nombre,
            tgt.rango_turno = src.rango_turno,
            tgt.updated_at = @now
    WHEN NOT MATCHED THEN
        INSERT (turno_codigo, turno_nombre, rango_turno)
        VALUES (src.turno_codigo, src.turno_nombre, src.rango_turno);

    ;WITH header_src AS (
        SELECT DISTINCT
            centro_codigo,
            linea_codigo,
            linea_nombre,
            especie_principal_linea
        FROM stg.FormularioHeader
        WHERE dw_loaded_at IS NULL
          AND NULLIF(LTRIM(RTRIM(centro_codigo)), '') IS NOT NULL
          AND NULLIF(LTRIM(RTRIM(linea_codigo)), '') IS NOT NULL
    )
    MERGE dw.DimLinea AS tgt
    USING header_src AS src
        ON tgt.centro_codigo = src.centro_codigo
       AND tgt.linea_codigo = src.linea_codigo
    WHEN MATCHED THEN
        UPDATE SET
            tgt.linea_nombre = COALESCE(src.linea_nombre, tgt.linea_nombre, src.linea_codigo),
            tgt.especie_principal = COALESCE(src.especie_principal_linea, tgt.especie_principal),
            tgt.updated_at = @now
    WHEN NOT MATCHED THEN
        INSERT (centro_codigo, linea_codigo, linea_nombre, especie_principal)
        VALUES (
            src.centro_codigo,
            src.linea_codigo,
            COALESCE(src.linea_nombre, src.linea_codigo),
            src.especie_principal_linea
        );

    ;WITH defect_src AS (
        SELECT DISTINCT
            codigo_defecto,
            COALESCE(nombre_defecto, codigo_defecto) AS nombre_defecto
        FROM stg.FormularioDefecto
        WHERE dw_loaded_at IS NULL
          AND NULLIF(LTRIM(RTRIM(codigo_defecto)), '') IS NOT NULL
    )
    MERGE dw.DimDefecto AS tgt
    USING defect_src AS src
        ON tgt.codigo_defecto = src.codigo_defecto
    WHEN MATCHED THEN
        UPDATE SET
            tgt.nombre_defecto = src.nombre_defecto,
            tgt.updated_at = @now
    WHEN NOT MATCHED THEN
        INSERT (codigo_defecto, nombre_defecto)
        VALUES (src.codigo_defecto, src.nombre_defecto);

    ;WITH fact_src AS (
        SELECT
            h.source_system,
            h.source_business_key,
            h.source_record_id,
            h.fecha,
            h.fecha_operacional,
            t.turno_key,
            c.centro_key,
            p.productor_key,
            e.especie_key,
            v.variedad_key,
            l.linea_key,
            ls.lugar_key,
            h.lote,
            h.verificador,
            h.observaciones,
            h.cant_muestra,
            h.suma_defectos,
            h.fruta_comercial,
            h.fruta_sana,
            h.choice,
            h.porc_exportable,
            h.porc_embalable,
            h.porc_choice,
            h.porc_descartable,
            h.porc_export_manual,
            h.velocidad_kgh,
            h.velocidad_manual,
            h.centro_sin_definir,
            h.estado_formulario,
            h.es_completo,
            h.campos_pendientes,
            h.created_at,
            h.updated_at
        FROM stg.FormularioHeader h
        INNER JOIN dw.DimTurno t
            ON t.turno_codigo = h.turno_codigo
        INNER JOIN dw.DimEspecie e
            ON e.especie_nombre = h.especie
        INNER JOIN dw.DimVariedad v
            ON v.especie_key = e.especie_key
           AND v.variedad_nombre = h.variedad
        INNER JOIN dw.DimLinea l
            ON l.centro_codigo = h.centro_codigo
           AND l.linea_codigo = h.linea_codigo
        LEFT JOIN dw.DimCentro c
            ON c.centro_codigo = h.centro_codigo
        LEFT JOIN dw.DimProductor p
            ON p.productor_codigo = h.productor_codigo
        LEFT JOIN dw.DimLugarSeleccion ls
            ON ls.lugar_codigo = h.lugar_codigo
        WHERE h.dw_loaded_at IS NULL
    )
    MERGE dw.FactFormulario AS tgt
    USING fact_src AS src
        ON tgt.source_system = src.source_system
       AND tgt.source_business_key = src.source_business_key
    WHEN MATCHED THEN
        UPDATE SET
            tgt.source_record_id = src.source_record_id,
            tgt.fecha = src.fecha,
            tgt.fecha_operacional = src.fecha_operacional,
            tgt.turno_key = src.turno_key,
            tgt.centro_key = src.centro_key,
            tgt.productor_key = src.productor_key,
            tgt.especie_key = src.especie_key,
            tgt.variedad_key = src.variedad_key,
            tgt.linea_key = src.linea_key,
            tgt.lugar_key = src.lugar_key,
            tgt.lote = src.lote,
            tgt.verificador = src.verificador,
            tgt.observaciones = src.observaciones,
            tgt.cant_muestra = src.cant_muestra,
            tgt.suma_defectos = src.suma_defectos,
            tgt.fruta_comercial = src.fruta_comercial,
            tgt.fruta_sana = src.fruta_sana,
            tgt.choice = src.choice,
            tgt.porc_exportable = src.porc_exportable,
            tgt.porc_embalable = src.porc_embalable,
            tgt.porc_choice = src.porc_choice,
            tgt.porc_descartable = src.porc_descartable,
            tgt.porc_export_manual = src.porc_export_manual,
            tgt.velocidad_kgh = src.velocidad_kgh,
            tgt.velocidad_manual = src.velocidad_manual,
            tgt.centro_sin_definir = src.centro_sin_definir,
            tgt.estado_formulario = src.estado_formulario,
            tgt.es_completo = src.es_completo,
            tgt.campos_pendientes = src.campos_pendientes,
            tgt.created_at = src.created_at,
            tgt.updated_at = src.updated_at,
            tgt.dw_updated_at = @now
    WHEN NOT MATCHED THEN
        INSERT (
            source_system,
            source_business_key,
            source_record_id,
            fecha,
            fecha_operacional,
            turno_key,
            centro_key,
            productor_key,
            especie_key,
            variedad_key,
            linea_key,
            lugar_key,
            lote,
            verificador,
            observaciones,
            cant_muestra,
            suma_defectos,
            fruta_comercial,
            fruta_sana,
            choice,
            porc_exportable,
            porc_embalable,
            porc_choice,
            porc_descartable,
            porc_export_manual,
            velocidad_kgh,
            velocidad_manual,
            centro_sin_definir,
            estado_formulario,
            es_completo,
            campos_pendientes,
            created_at,
            updated_at
        )
        VALUES (
            src.source_system,
            src.source_business_key,
            src.source_record_id,
            src.fecha,
            src.fecha_operacional,
            src.turno_key,
            src.centro_key,
            src.productor_key,
            src.especie_key,
            src.variedad_key,
            src.linea_key,
            src.lugar_key,
            src.lote,
            src.verificador,
            src.observaciones,
            src.cant_muestra,
            src.suma_defectos,
            src.fruta_comercial,
            src.fruta_sana,
            src.choice,
            src.porc_exportable,
            src.porc_embalable,
            src.porc_choice,
            src.porc_descartable,
            src.porc_export_manual,
            src.velocidad_kgh,
            src.velocidad_manual,
            src.centro_sin_definir,
            src.estado_formulario,
            src.es_completo,
            src.campos_pendientes,
            src.created_at,
            src.updated_at
        );

    ;WITH defect_src AS (
        SELECT
            f.formulario_key,
            d.defecto_key,
            s.cantidad,
            s.updated_at
        FROM stg.FormularioDefecto s
        INNER JOIN dw.FactFormulario f
            ON f.source_system = s.source_system
           AND f.source_business_key = s.source_business_key
        INNER JOIN dw.DimDefecto d
            ON d.codigo_defecto = s.codigo_defecto
        WHERE s.dw_loaded_at IS NULL
    )
    MERGE dw.FactFormularioDefecto AS tgt
    USING defect_src AS src
        ON tgt.formulario_key = src.formulario_key
       AND tgt.defecto_key = src.defecto_key
    WHEN MATCHED THEN
        UPDATE SET
            tgt.cantidad = src.cantidad,
            tgt.updated_at = src.updated_at,
            tgt.dw_updated_at = @now
    WHEN NOT MATCHED THEN
        INSERT (formulario_key, defecto_key, cantidad, updated_at)
        VALUES (src.formulario_key, src.defecto_key, src.cantidad, src.updated_at);

    ;WITH forms_in_batch AS (
        SELECT DISTINCT
            f.formulario_key
        FROM stg.FormularioHeader h
        INNER JOIN dw.FactFormulario f
            ON f.source_system = h.source_system
           AND f.source_business_key = h.source_business_key
        WHERE h.dw_loaded_at IS NULL
    )
    DELETE tgt
    FROM dw.FactFormularioDefecto tgt
    INNER JOIN forms_in_batch b
        ON b.formulario_key = tgt.formulario_key
    WHERE NOT EXISTS (
        SELECT 1
        FROM stg.FormularioDefecto s
        INNER JOIN dw.DimDefecto d
            ON d.codigo_defecto = s.codigo_defecto
        INNER JOIN dw.FactFormulario f
            ON f.source_system = s.source_system
           AND f.source_business_key = s.source_business_key
        WHERE s.dw_loaded_at IS NULL
          AND f.formulario_key = tgt.formulario_key
          AND d.defecto_key = tgt.defecto_key
    );

    UPDATE stg.FormularioHeader
    SET dw_loaded_at = @now
    WHERE dw_loaded_at IS NULL;

    UPDATE stg.FormularioDefecto
    SET dw_loaded_at = @now
    WHERE dw_loaded_at IS NULL;
END
GO
